...
st.title("🚆 TransNational Railways Incident Prediction System")
st.markdown("""
...
# Initialize monitoring in session state
...
with st.sidebar:
    ...
    if len(st.session_state.prediction_log) > 0:
        ...
        st.metric("Incident Rate", f"{log_df['prediction'].mean():.1%}")
        st.metric("Avg Confidence", f"{log_df['probability'].mean():.1%}")
        
        # Alert checks
        if log_df['prediction'].mean() > 0.25:
            st.warning("⚠️ High incident rate detected")
        if log_df['prediction'].mean() < 0.05:
            st.warning("⚠️ Low incident rate detected")
        if log_df['probability'].mean() < 0.6:
            st.warning("⚠️ Low confidence scores")
        
        # Show recent predictions
        st.subheader("Recent Predictions")
        recent = log_df.tail(5)[['timestamp', 'prediction', 'probability']].copy()
        recent['prediction'] = recent['prediction'].map({0: 'No Incident', 1: 'Incident'})
...
# Time Information
with st.expander("🕐 Time Information", expanded=True):
...
with st.expander("🚄 Train Information", expanded=True):
    ...
    st.info("ℹ️ Train Class: Azuma (constant for all services)")
...
# Booking Information
with st.expander("📊 Booking & Forecast Information", expanded=False):
    col1, col2 = st.columns(2)
...
# Prediction button
if st.button("🔮 Predict Incident Risk", type="primary", use_container_width=True):
    ...
    with col2:
        if prediction == 1:
            ...
            st.success("✅ NO INCIDENT PREDICTED")
            st.markdown(f"""
